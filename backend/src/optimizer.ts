...
export function optimizeActionPlan(data: DataObject[]): ActionEntry[] {
  ...
  for (const entry of data) {
      ...
      if (potentialBuyHours.has(`${hour}-${day}`) && (lastBuyHour === null || hour !== lastBuyHour)) {
          const potentialSellHoursOfDay = Array.from(potentialSellHours).filter(h => h.endsWith(day) && h.split('-')[0] > hour);
          if (potentialSellHoursOfDay.length > 0) {
              const chargeAmountKwh = Math.min(chargingSpeedKwh, batteryCapacity * maxBattery - batteryKwh);
              batteryKwh += chargeAmountKwh;
              action = 'BUY';
              highestBuyPrice = Math.max(highestBuyPrice, price);
              lastBuyHour = hour;
          }
      } else if (potentialSellHours.has(`${hour}-${day}`) || (highestBuyPrice && price > highestBuyPrice + minProfitThreshold)) {
          sellingPrice = price;
          const profitThreshold = highestBuyPrice + minProfitThreshold;
          if (sellingPrice > profitThreshold && lastBuyHour !== hour) {
              const dischargeAmountKwh = Math.min(chargingSpeedKwh, batteryKwh - batteryCapacity * minBattery);
